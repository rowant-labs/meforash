"""Bounded arm E: original-text weights plus the frozen v1 SFT at LR 0.00002.

Planning is offline. Execution additionally requires the fresh evaluation freeze.
The v1 preparation, training implementation, and completed runs are preserved.
"""
...
ROOT = raw.ROOT
MANIFEST = "manifests/instruction-calibration-training-v2.json"
EXPERIMENT = "manifests/instruction-calibration-experiment-v2.json"
PREPARATION_SHA256 = "b851caf978f50c63ef5f2cba856e74f46ab751090ee47401100fcf9cff8b34cc"
REFERENCE_RECIPE_SHA256 = "e4e2490a2951d90a498e03412456b7b2059bf2b521448ac12dde59dd26928303"
ORDER_SHA256 = "1ef94b22549d97edd35116d4f8680c616d38772e1cd4367908ec7ff3e3a8709f"
PLANNED_NANO = 3481495080
...
class Config:
    ...
    def validate(self):
        if (self.arm != "E" or self.batch_size != 16 or self.seed != 20260906 or self.learning_rate != 2e-5
                or self.checkpoint_ttl_seconds != 30 * 86400
                or self.checkpoint_reserve_usd != 3.0 or self.timeout_seconds != 600.0):
            raise TrainingError("Arm E fixes the matched recipe, LR 0.00002, thirty-day checkpoints, $3 contingency, and 600-second deadlines.")
        self.reference_config().validate()

    def reference_config(self):
        # This ordinary call verifies the original frozen recipe first. It does
        # not execute D or alter any imported module/configuration.
        return v1.Config(arm="D", source_checkpoint_file=self.source_checkpoint_file,
                         max_cost_usd=self.max_cost_usd, timeout_seconds=self.timeout_seconds)
...
def load_training_protocol(root=ROOT):
    ...
    if (protocol.get("schema_version") != 2 or protocol.get("status") != "frozen_training_recipe"
            or protocol.get("experiment_id") != "instruction-calibration-v2"
            or protocol.get("arm") != "E" or protocol.get("parent_arm") != "B"
            or protocol.get("changed_training_parameter") != "adam.learning_rate"
            or protocol.get("reference_recipe_sha256") != REFERENCE_RECIPE_SHA256
            or raw.digest(raw.encoded(protocol.get("reference_recipe"))) != REFERENCE_RECIPE_SHA256
            or protocol.get("preparation") != {"path": v1.MANIFEST, "sha256": PREPARATION_SHA256}
            or protocol.get("sequence_order_sha256") != ORDER_SHA256
            or protocol.get("planned_nano_usd") != PLANNED_NANO
            or protocol.get("checkpoint_reserve_nano_usd") != 3 * raw.NANO
            or protocol.get("checkpoint_ttl_seconds") != 30 * 86400):
        raise TrainingError("Arm E training protocol differs from the fixed calibration.")
    expected = copy.deepcopy(protocol["reference_recipe"])
    expected["adam"]["learning_rate"] = 2e-5
    if protocol.get("recipe") != expected or protocol.get("recipe_sha256") != raw.digest(raw.encoded(expected)):
        raise TrainingError("Only the fivefold learning-rate reduction may differ from v1.")
    expected_counts = {"training_sequences": 104, "training_batches": 7,
                       "training_processed_tokens": 64154, "training_loss_tokens": 15554,
                       "holdout_sequences": 16, "two_holdout_forward_tokens": 21674}
    if protocol.get("counts") != expected_counts:
        raise TrainingError("The calibration must retain the exact v1 training and validation sizes.")
    v1.checked_file(root, protocol["preparation"], prefix="manifests")
    software = protocol.get("software_sha256", {})
    required = {"bibleprep/train_instruction_calibration.py", "bibleprep/train_instruction.py",
                "bibleprep/train_inkling.py", "bibleprep/environment.py",
                "requirements-training.txt", "requirements-comparison-large.lock.txt"}
    if set(software) != required:
        raise TrainingError("The calibration requires all frozen implementation identities.")
    for name, expected_hash in software.items():
        if raw.digest(v1.project_file(root, name).read_bytes()) != expected_hash:
            raise TrainingError("Calibration software changed after its freeze.")
...
def make_plan(config, root=ROOT):
    ...
    reference, batches, holdout = v1.make_plan(config.reference_config(), root)
    if (reference["recipe_sha256"] != REFERENCE_RECIPE_SHA256
            or reference["recipe"] != protocol["reference_recipe"]
            or reference["source_adapter"] != protocol["source_adapter"]
            or reference["sequence_order_sha256"] != ORDER_SHA256
            or reference["planned_nano_usd"] != PLANNED_NANO
            or any(reference.get(key) != value for key, value in protocol["counts"].items())):
        raise TrainingError("The verified parent, dataset, order, or cost differs from the calibration freeze.")
    plan = {**reference, "schema_version": 2, "phase": "instruction_calibration", "arm": "E",
            "recipe": copy.deepcopy(protocol["recipe"]), "recipe_sha256": protocol["recipe_sha256"],
            "training_protocol_sha256": protocol_hash,
            "software_sha256": protocol["software_sha256"],
            "reference_instruction_recipe_sha256": REFERENCE_RECIPE_SHA256,
            "source_checkpoint_file": config.source_checkpoint_file,
            "parent_arm": "B", "changed_training_parameter": "adam.learning_rate",
            "learning_rate_status": "experimental_fivefold_reduction_not_official_optimized_rate",
            "expected_combined_training_reserved_estimated_usd": raw.money(v1.reserved_budget(root) + PLANNED_NANO),
            "execution_requires_fresh_evaluation_freeze": True}
    plan.pop("pair_planned_estimated_usd")
    return plan, batches, holdout


def verify_evaluation_freeze(root=ROOT):
    """Hash evaluation bytes and inspect freeze metadata; never parse questions."""
    training, training_hash = load_training_protocol(root)
    ...
    if (experiment.get("schema_version") != 2
            ...
            or experiment.get("experiment_id") != "instruction-calibration-v2"
            or experiment.get("training_manifest") != {"path": MANIFEST, "sha256": training_hash}
            or experiment.get("arms", {}).get("E") != "original_text_then_instruction_lr_0.00002"):
        raise TrainingError("Fresh calibration evaluation must be frozen before training.")
    evaluation = experiment["evaluation"]
    if (evaluation.get("path") != "evals/instruction-calibration-v2.jsonl"
        ...
        raise TrainingError("The calibration requires the frozen thirty-case evaluation.")
    v1.checked_file(root, evaluation, prefix="evals")
    metadata_entry = experiment["evaluation_manifest"]
    if metadata_entry.get("path") != "manifests/instruction-calibration-evaluation-v2.json":
        raise TrainingError("The calibration evaluation freeze manifest is not the expected version.")
    metadata = json.loads(v1.checked_file(root, metadata_entry, prefix="manifests"))
    if (metadata.get("schema_version") != 2
            or metadata.get("status") != "frozen_before_training_and_sampling"
...
class CalibrationNativeSession(v1.InstructionNativeSession):
    """E uses the exact weights-only initialization already exercised by D.
    ...
    def call(self, operation, rows, settings):
        if settings.get("arm") != "E" or settings.get("adam", {}).get("learning_rate") != 2e-5:
            raise TrainingError("The calibration transport only accepts the frozen E recipe.")
        return super().call(operation, rows, {**settings, "arm": "D"})


def calibration_worker(connection):
...
def run(config, *, execute=False, root=ROOT, transport=None):
    ...
    directory = raw.private_path(root, config.run_dir or "runs/inkling-instruction-calibration-" + config.arm.lower() + "-" + slug)
    ...
    try:
        if native is None:
            ...
            native = raw.BoundedTrainingTransport(worker=calibration_worker)
        invoke("create")
        ...
        for operation, key in (("state", "training_state_path"), ("sampler", "sampler_path")):
            settings["checkpoint_name"] = "instruction-calibration-" + config.arm.lower() + "-" + operation
        ...
        summary = {"status": "complete", "phase": "instruction_calibration", "arm": config.arm, "run_id": run_id,
        ...
        raw.write_private(directory / "summary.json", {"status": "stopped_uncertain", "phase": "instruction_calibration", "arm": config.arm,
...
def _check_metrics(metrics, rows):
    if not isinstance(metrics, dict):
        raise TrainingError("The completed calibration lacks aligned validation metrics.")
    ...
    if (metrics.get("processed_tokens") != sum(len(row["input_ids"]) for row in rows)
        ...
        raise TrainingError("Completed calibration metrics disagree with the exact target counts.")
...
def verify_completed_checkpoint(root, checkpoint_file):
    ...
    try:
        ...
        raise TrainingError("Arm E checkpoint failed its frozen calibration receipt verification.") from None
...
def _verify_completed_checkpoint(root, checkpoint_file):
    ...
    if path.name != "checkpoints.json":
        raise TrainingError("Use the completed calibration checkpoints.json file.")
    ...
    if (not isinstance(run_id, str) or not run_id or summary.get("run_id") != run_id
            or summary.get("status") != "complete" or plan.get("schema_version") != 2
            or any(obj.get("phase") != "instruction_calibration" or obj.get("arm") != "E"
        ...
        raise TrainingError("Completed calibration plan and summary disagree with the frozen recipe.")
    source = v1.source_receipt(Config(source_checkpoint_file=plan["source_checkpoint_file"]), root)[0]
    if source != protocol["source_adapter"]:
        raise TrainingError("The completed calibration did not initialize from the exact B parent.")
    for key, marker in (("training_state_path", "/weights/"), ("sampler_path", "/sampler_weights/")):
        ...
        if not isinstance(value, str) or not re.fullmatch(r"tinker://[A-Za-z0-9._:/-]+", value) or marker not in value:
            raise TrainingError("The completed calibration must retain both checkpoint kinds.")
    from bibleprep.prepare_instruction import verify_prepared
    import random
    prepared = json.loads(v1.checked_file(root, protocol["preparation"], prefix="manifests"))
    rows = verify_prepared(root, prepared)
    selected = sorted(rows["train"], key=lambda row: row["id"])
    random.Random(20260906).shuffle(selected)
    if raw.digest(raw.encoded([row["id"] for row in selected])) != ORDER_SHA256:
        raise TrainingError("The completed calibration sequence order cannot be reproduced.")
    ...
    if (len(events) != 2 * len(operations) + 1
        ...
        raise TrainingError("The calibration journal is missing, uncertain, or contains extra operations.")
    losses = []
    for index, (operation, batch, stage, step, budget) in enumerate(operations, 1):
        ...
        if ({key: value for key, value in reserve.items() if key != "time_utc"}
            ...
            raise TrainingError("The calibration journal does not prove every planned operation completed once.")
        ...
        if batch:
            ...
            if set(metrics.get("by_language", {})) != languages:
                raise TrainingError("Calibration language metrics are incomplete.")
            for language in languages:
            ...
            if not math.isclose(sum(v["weighted_loss_sum"] for v in metrics["by_language"].values()),
                ...
                raise TrainingError("Calibration language losses do not reconcile.")
            if operation == "forward" and summary.get(stage) != metrics:
                raise TrainingError("Saved calibration validation metrics differ from their journal.")
            if operation == "forward_backward":
                losses.append(metrics)
        elif metrics is not None:
            raise TrainingError("A non-forward calibration operation unexpectedly contains loss metrics.")
    ...
    if not math.isclose(summary["training_weighted_nll"], nll, rel_tol=1e-10, abs_tol=1e-10):
        raise TrainingError("Calibration training diagnostic differs from its journal.")
    ...
    if len(reservations) != 1 or reservations[0]["amount_nano_usd"] != PLANNED_NANO:
        raise TrainingError("The calibration has no unique matching shared-budget reservation.")
...
def main(argv=None):
    ...
    try:
        ...
        print("Calibration planning or training could not complete; no automatic retry was attempted. Inspect the private receipt if execution began.")
        return 1
    return 0
