...
The [v4 cache test](SMALL-MODEL-SERVING-RESULTS-V4.md) reached pinned-model execution: 136.7 seconds from request submission to completion and a 0.94-second warm probe. The [v5 idle follow-up](SMALL-MODEL-SERVING-RESULTS-V5.md) completed startup near its six-minute stop boundary, without retaining the answer or reaching idle measurement. Fresh startup remains variable; the observed 90-second v4 idle interval stayed billable. Stop unchanged retries and reconsider preparation/hosting strategy before making production latency or scale-to-zero cost claims.

The [startup alternatives review](SMALL-MODEL-STARTUP-ALTERNATIVES.md) recommends a controlled Modal cached/snapshot comparison next, with Baseten as the managed alternative. Both remain untested here; GPU snapshot alpha status, actual shutdown, full resource cost and future adapter restoration are explicit gates.
