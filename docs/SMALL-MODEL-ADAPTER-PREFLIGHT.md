...
2. The upstream community fixture has no declared license. Use the project-created synthetic rank-8 fixture instead, with its exact real layer dimensions and hashes recorded in [loader preflight](MODAL-ADAPTER-LOADER-PREFLIGHT.md). This supersedes the earlier proposed requirement to execute the community fixture first.
3. Prepare one bounded GPU loader probe with the pinned native-MXFP4 base. Start at 1,024 tokens, then test the project's 8,192-token setting only within the declared budget and after successful loading. Require complete tensor accounting, enforced adapter selection including an unknown-alias negative control, memory/timing receipts and cleanup. A subtle synthetic adapter may leave greedy text unchanged; compare matched log probabilities and record an inconclusive numerical effect honestly. No real Tinker export or Bible quality conclusion follows.
