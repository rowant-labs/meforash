"""Versioned v2 Modal pilot (`tools/modal_serving_pilot_v2.py`).
...
VOLUME_NAME = "meforash-modal-serving-v2-weights"
...
APP_NAME = (
    "meforash-modal-serving-v2-snapshot"
    if SNAPSHOT_ENABLED
    else "meforash-modal-serving-v2-baseline"
)
...
class Model:
    ...
    def _timed_startup_phase(self, phase: str) -> Iterator[None]:
        ...
        try:
            ...
            print(
                json.dumps(
                    {
                        "event": "modal_serving_pilot_v2_phase",
                        "phase": phase,
                        "status": "failed",
    ...
    def start(self) -> None:
        ...
        if SNAPSHOT_ENABLED:
            ...
            print(
                json.dumps(
                    {
                        "event": "modal_serving_pilot_v2_snapshot_capture_marker",
                        "phase": "post_sleep",
    ...
    def remote_gen(
        ...
        """Yield only final-channel text deltas, then public usage and status."""

        yield from _iter_completion(
            nonce,
            prompt,
            max_tokens=max_tokens,
            reasoning_effort=reasoning_effort,
