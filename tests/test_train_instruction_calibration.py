...
from bibleprep import train_instruction_calibration as subject
...
def public_fixture(root):
    protocol = json.loads((subject.ROOT / subject.MANIFEST).read_text())
    for name in [subject.MANIFEST, v1.MANIFEST, *protocol["software_sha256"]]:
        path = root / name
        path.parent.mkdir(parents=True, exist_ok=True)
...
def frozen_evaluation(root):
    data = root / "evals/instruction-calibration-v2.jsonl"
    ...
    metadata = root / "manifests/instruction-calibration-evaluation-v2.json"
    write(metadata, {"schema_version": 2, "status": "frozen_before_training_and_sampling", "dataset": dataset})
    experiment = {"schema_version": 2, "status": "frozen_before_training_and_sampling",
                  "experiment_id": "instruction-calibration-v2",
                  "arms": {"E": "original_text_then_instruction_lr_0.00002"}, "evaluation": dataset,
...
class FakeTransport:
    ...
    def __call__(self, operation, rows, settings):
        ...
        assert settings["arm"] == "E" and settings["adam"]["learning_rate"] == 2e-5
        ...
        if operation in {"state", "sampler"}:
            ...
            return {"path": "tinker://synthetic/" + folder + "/calibration"}
        return {}
...
class CalibrationTests(TestCase):
    def test_only_fivefold_lr_reduction_is_configurable_recipe(self):
    ...
    def test_planner_reuses_exact_verified_D_inputs_and_changes_only_learning_rate(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            protocol = public_fixture(root)
            reference = {"recipe": protocol["reference_recipe"],
                         "recipe_sha256": protocol["reference_recipe_sha256"],
                         "source_adapter": protocol["source_adapter"],
                         "sequence_order_sha256": protocol["sequence_order_sha256"],
                         "planned_nano_usd": subject.PLANNED_NANO,
                         "pair_planned_estimated_usd": 6.96299016, **protocol["counts"]}
            batches, holdout = [[copy.deepcopy(ROW)]], [copy.deepcopy(ROW)]
            with patch.object(v1, "make_plan", return_value=(reference, batches, holdout)) as verification:
                plan, actual_batches, actual_holdout = subject.make_plan(subject.Config(), root)
            passed_config = verification.call_args.args[0]
            self.assertEqual(passed_config.arm, "D")
            self.assertEqual(passed_config.learning_rate, 1e-4)
            self.assertEqual(passed_config.source_checkpoint_file, subject.Config().source_checkpoint_file)
            expected_recipe = copy.deepcopy(protocol["reference_recipe"])
            expected_recipe["adam"]["learning_rate"] = 2e-5
            self.assertEqual(plan["recipe"], expected_recipe)
            self.assertIs(actual_batches, batches)
            self.assertIs(actual_holdout, holdout)
            self.assertEqual(reference["recipe"]["adam"]["learning_rate"], 1e-4)
            for field, value in (("sequence_order_sha256", "wrong"), ("source_adapter", {"wrong": "D parent"})):
                with patch.object(v1, "make_plan", return_value=({**reference, field: value}, batches, holdout)), self.assertRaises(raw.TrainingError):
                    subject.make_plan(subject.Config(), root)

    def test_D_instruction_receipt_is_not_accepted_as_E_original_text_parent(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
    ...
    def test_freeze_gate_hashes_evaluation_without_parsing_questions(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            (root / "evals/instruction-calibration-v2.jsonl").write_text("Changed evaluation bytes")
            with self.assertRaises(raw.TrainingError):
    ...
    def test_weights_only_initialization_preserves_caller_settings_and_rejects_repeat(self):
        session = subject.CalibrationNativeSession()
        session.client, session.source_verified = MagicMock(), True
        settings = {"arm": "E", "adam": {"learning_rate": 2e-5},
    ...
    def test_no_training_before_verified_B_weights_load(self):
        for operation in ("forward", "forward_backward", "optim", "load_weights"):
            ...
            with self.subTest(operation=operation), self.assertRaises(raw.TrainingError):
                session.call(operation, [], {"arm": "E", "adam": {"learning_rate": 2e-5}})
...
class RunTests(TestCase):
    def setUp(self):
        ...
        self.output = self.root / "runs/synthetic-e"
        ...
        self.protocol = json.loads((subject.ROOT / subject.MANIFEST).read_text())
        ...
        self.plan = {"schema_version": 2, "phase": "instruction_calibration", "arm": "E", "parent_arm": "B",
                     "recipe": self.protocol["recipe"], "recipe_sha256": self.protocol["recipe_sha256"],
                     "source_adapter": self.protocol["source_adapter"], "source_checkpoint_file": "runs/synthetic-b/checkpoints.json",
                     ...
                     "planned_nano_usd": subject.PLANNED_NANO, "planned_estimated_usd": 3.48149508,
        ...
        self.config = subject.Config(run_dir="runs/synthetic-e")
    ...
    def test_seven_updates_reserve_before_calls_and_preserve_shifted_arrays(self):
        ...
        raw.reserve_budget(self.root, "synthetic-prior", 31953737760, raw.HARD_CAP_NANO)
        ...
        self.assertEqual(summary["combined_reserved_estimated_usd"], 35.43523284)
    ...
    def test_checkpoint_verifier_binds_parent_order_and_entire_journal(self):
        ...
        with patch.object(subject, "load_training_protocol", return_value=(self.protocol, "a" * 64)), \
                ...
                patch("bibleprep.prepare_instruction.verify_prepared", return_value={"train": self.train, "validation": self.holdout}):
            sampler, digest = subject.verify_completed_checkpoint(self.root, self.output / "checkpoints.json")
            self.assertEqual(sampler, "tinker://synthetic/sampler_weights/calibration")
